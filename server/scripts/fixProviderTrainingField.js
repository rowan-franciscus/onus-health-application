...
async function fixProviderTrainingField() {
  try {
    ...
    await mongoose.connect(
      process.env.MONGODB_URI || "mongodb://localhost:27017/onus-health",
      {
    ...
    const providers = await User.find({
      role: "provider",
      "providerProfile.supportPreferences.requiresTraining": {
        $type: "string",
      },
    });

    logger.info(
      `Found ${providers.length} providers with string requiresTraining values`,
    );
    ...
    for (const provider of providers) {
      ...
      logger.info(
        `Processing provider ${provider.email} with requiresTraining value: "${currentValue}"`,
      );
      ...
      if (currentValue && typeof currentValue === "string") {
        ...
        booleanValue = /^(yes|y|true|1|require|need|would like|please)/i.test(
          currentValue.trim(),
        );
      }
      ...
      logger.info(
        `Fixed provider ${provider.email} - requiresTraining set to: ${booleanValue}`,
      );
    }
    ...
    if (providersWithMissingStructure.length > 0) {
      logger.info(
        `Found ${providersWithMissingStructure.length} providers with missing supportPreferences structure`,
      );
      ...
      for (const provider of providersWithMissingStructure) {
        ...
        if (!provider.providerProfile.supportPreferences) {
          ...
          await provider.save();
          logger.info(
            `Added supportPreferences structure for provider ${provider.email}`,
          );
        }
      }
    }
