...
const ProviderViewVitals = () => {
  ...
  const handleBackClick = () => {
    // Navigate back to previous page or medical records
    if (window.history.length > 1) {
      navigate(-1);
    } else {
      navigate('/provider/medical-records');
    }
  };
  ...
  return (
    <PageContainer>
      <div className={styles.header}>
        <div>
          ...
          <p className={styles.patientInfo}>
            Patient: {vitalsRecord.patient?.firstName} {vitalsRecord.patient?.lastName}
          </p>
          {vitalsRecord.createdByPatient && (
            <span className={styles.selfRecordedBadge}>Patient Self-Recorded</span>
          )}
        </div>
        <Button 
          variant="secondary" 
      ...
      {vitalsRecord.provider && !vitalsRecord.createdByPatient && (
...
export default ProviderViewVitals; 